...
- **Hunger Corpse System** - Dino Den-inspired feature that spawns corpses when players get hungry
...
- Configurable hunger threshold (0-100%)
- Per-player cooldown system
- Carnivore-only filter option
- Customizable spawn radius (min/max)
- Species selection with size matching
- Performance controls (check interval, max players per check)
- Prevents exploits with distance and timing limits
